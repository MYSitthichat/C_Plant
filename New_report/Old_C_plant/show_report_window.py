...
class ReportWindowApp:
    ...
    def show_report_button_pressed(self):
        ...
        for present_data in response_list:
            ...
            db_connector = sqlite3.connect(database_path)
            db_cursor = db_connector.cursor()
            sql_query = "SELECT * FROM booking_table WHERE Booking_ID == " + str(present_data[0]) + ";"
            db_cursor.execute(sql_query)
            detail_response = db_cursor.fetchall()
            db_connector.close()
            detail_description = "จัดส่งที่" + detail_response[0][4] + " รถทะเบียน " + detail_response[0][10]

            # ===============================================================================
            db_connector = sqlite3.connect(database_path)
            db_cursor = db_connector.cursor()
            sql_query = "SELECT * FROM concrete_formula_table WHERE Formula_ID == " + str(detail_response[0][8]) + ";"
            db_cursor.execute(sql_query)
            formula_name_response = db_cursor.fetchall()
            db_connector.close()
            # ===============================================================================
            error1 = (int(present_data[14]) - int(present_data[6]))/int(present_data[6])*100
            error1_float = float("{:.2f}".format(error1))
            first_row = (present_data[0],present_data[2],detail_description,formula_name_response[0][1],present_data[1],present_data[4],"หิน 1",present_data[6],present_data[14],error1_float)
    ...
    def export_button_pressed(self):
        ...
        self.total_record_number_int.set(len(response_list))
        display_list = []
        headerList = ["วันที่","เวลา","ลำดับ","ชื่อลูกค้า","ทะเบียนรถ","รายละเอียด","กำลังอัด","จำนวนคิว","หิน1(กำหนด)","หิน2(กำหนด)"
              ,"ทราย(กำหนด)","ปูนซีเมนต์(กำหนด)","เถ้าลอย(กำหนด)","น้ำ(กำหนด)","น้ำยาเคมี1(กำหนด)","น้ำยาเคมี2(กำหนด)",
              "หิน1(วัดจริง)","หิน2(วัดจริง)","ทราย(วัดจริง)","ปูนซีเมนต์(วัดจริง)","เถ้าลอย(วัดจริง)","น้ำ(วัดจริง)","น้ำยาเคมี1(วัดจริง)","น้ำยาเคมี2(วัดจริง)"]
        for present_data in response_list:
            #==================================================================================================
            db_connector = sqlite3.connect(database_path)
            db_cursor = db_connector.cursor()
            ...
            detail_response = db_cursor.fetchall()
            db_connector.close()
            detail_description = "จัดส่งที่" + detail_response[0][4]
            #==================================================================================================
            db_connector = sqlite3.connect(database_path)
            db_cursor = db_connector.cursor()
            sql_query = "SELECT * FROM concrete_formula_table WHERE Formula_ID == " + str(detail_response[0][8]) + ";"
            ...
            db_connector.close()
            ##==================================================================================================
            record_date,record_time = present_data[1].split(" ")
            display_list.append([record_date,record_time,present_data[0],present_data[2],detail_response[0][10],detail_description,formula_name_response[0][1],present_data[4],present_data[6], \
                                 present_data[8],present_data[7],present_data[9],present_data[10],present_data[11],present_data[12],present_data[13], \
                                 present_data[14],present_data[16],present_data[15],present_data[17],present_data[18],present_data[19],present_data[20],present_data[21],])

        # export_file_name = "d:/From" + str(self.start_date_dateEntry.get_date()) + "To" + str(self.end_date_dateEntry.get_date()) +".csv"

        export_file_name = asksaveasfile(initialfile = 'Untitled.csv', defaultextension=".csv",filetypes=[("CSV file","*.csv")])
        if export_file_name:
            with open(export_file_name.name, 'w', newline='') as csvfile:
                fdw = write = csv.writer(csvfile)
                fdw.writerow(headerList )
                fdw.writerows(display_list)
