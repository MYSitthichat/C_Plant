...
class AUTODA_Controller(QThread,QObject):
    ...
    def connect_to_autodac(self):
        # Debug 
        self.autoda_port = 'COM4'
        self.baudrate = '9600'
        self.stop_bits = '1'
        self.parity = 'N'
        self.data_bits = '8'
        self.timeout_error = '3'
        # Debug
        ...
        print(self.autoda_client.is_socket_open())
        try:
    ...
    @Slot()
    def read_setpoint_rock_and_sand(self):
        address_register = 314
        address_rock_and_sand = 1
        read_setpoint = self.autoda_client.read_holding_registers(address=address_register, count=2, device_id=address_rock_and_sand)
        result = read_setpoint.registers
        self.setpoint_rock_sand_read.emit(result)
        # return result
    
    def read_setpoint_cement_and_fyash(self):
        address_register = 314
        address_cement_and_fyash = 2
        read_setpoint = self.autoda_client.read_holding_registers(address=address_register, count=2, device_id=address_cement_and_fyash)
        result = read_setpoint.registers
        # return result

    def read_setpoint_water(self):
        address_register = 314
        address_water = 3
        read_setpoint = self.autoda_client.read_holding_registers(address=address_register, count=2, device_id=address_water)
        result = read_setpoint.registers
        # return result

    def read_setpoint_chemical(self):
        address_register = 314
        address_chem = 4
        read_setpoint = self.autoda_client.read_holding_registers(address=address_register, count=2, device_id=address_chem)
        result = read_setpoint.registers
        # return result
