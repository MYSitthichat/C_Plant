        ...
        while self.main_condition_load_running:
            try:
                ...
                if self.state_main_condition_load == 0:
                    # self.plc_controller.clear_memory()
            ...
            # STATE 2: รอ Sand โหลดเสร็จ
                ...
                if self.is_sand_frozen:
                    ...
                    if original_rock1 > 0:
                        ...
                        # self.new_offset = self.this_weight_to_new_offset
                        # rock_1_new = int(rock_1_new) + (self.new_offset)
                        # self.autoda_controller.write_set_point_rock_and_sand(rock_1)
                        # actual_sand_weight = int(self.main_window.mix_monitor_sand_lineEdit.text())
                        # rock1_setpoint = actual_sand_weight + original_rock1 - self.rock1_offset
                        # self.status_message.emit(f"ค่า setpoint rock1 = {rock1_setpoint} Display Now = {actual_sand_weight}  ค่าตามสูตร = {original_rock1}  offset = {self.rock1_offset}")
                        self.autoda_controller.write_set_point_rock_and_sand(rock_1)
                    time.sleep(1)
                    self.state_load_rock_and_sand = 3
            ...
            # STATE 3: เริ่มโหลด Rock1
                ...
                if original_rock1 <= 0:
                    ...
                    if original_rock2 > 0:
                        ...
                        # actual_sand_weight = int(self.main_window.mix_monitor_sand_lineEdit.text())
                        # rock2_setpoint = actual_sand_weight + original_rock2 - self.rock2_offset
                        # self.status_message.emit(f"ค่า setpoint rock2 = {rock2_setpoint} Display Now = {actual_sand_weight}  ค่าตามสูตร = {original_rock2}  offset = {self.rock2_offset}")
            ...
            # STATE 4: รอ Rock1 โหลดเสร็จ
                ...
                if self.is_rock1_frozen:
                    ...
                    if original_rock2 > 0:
                        ...
                        # actual_rock1_weight = int(self.main_window.mix_monitor_rock_1_lineEdit.text())
                        # rock2_setpoint = actual_rock1_weight + original_rock2 - self.rock2_offset
                        # self.status_message.emit(f"ค่า setpoint rock2 = {rock2_setpoint} Display Now = {actual_rock1_weight}  ค่าตามสูตร = {original_rock2}  offset = {self.rock2_offset}")
                        self.autoda_controller.write_set_point_rock_and_sand(rock_2)
                    time.sleep(1)
                    self.state_load_rock_and_sand = 5
    ...
    def load_cement_and_fyash_sequence(self,data_loaded):
        ...
        while self.is_loading_cement_and_fyash_in_progress:
            ...
            # STATE 3: เริ่มโหลด Cement
                ...
                cutoff_offset = 10  # กำหนด offset สำหรับการหยุดโหลดรอบแรก
                ...
                if abs(self.diff) <= 2:
                    ...
                    if self.retry_count > 5:  # ยอมให้เติมทีละนิดได้ไม่เกิน 5 ครั้ง
                ...
                self.status_message.emit("กำลังเติมปูนทีละ 1.5 วินาที")
                time.sleep(0.5)
                self.plc_controller.loading_cement("start")
                time.sleep(1.5)
                self.plc_controller.loading_cement("stop")
