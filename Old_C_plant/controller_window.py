...
client = ModbusClient(method='rtu',port='/dev/ttyUSB0',stopbitd=1,bytesize=8,parity='N',baudrate=9600,timeout=1)
...
def main_controller():
    ...
    global chem_offset_float
    try:
        message = "run at state " + str(main_state)
        add_status(message)
        if not running:
            print(main_state)
        if main_state == 0:
            ...
            add_status("read chem offset: "+str(chem_offset_float))
            main_state = 1
            if in_loop:
        ...
        # set sand weights to PLC1
            ...
            sand_float = float(sand_target_weight_string.get())*0.22 + 296              # y = 0.22x + 289
        ...
        # set rock1 weights to PLC1 (rock1 + sand)
            ...
            rock1_weight_int = int((s1+r1)*0.22 + 290)                                  # y = 0.22x + 289
            if running:
        ...
        # set rock2 weights to PLC1 (rock1 + rock2 + sand)
            ...
            rock2_weight_int = int((s1+r1+r2)*0.22 + 290)                               # y = 0.22x + 289
            if running:
        ...
        # ========= set flyash weight ==============
            ...
            if current_amount <= 0.5:
                flyash_weight_int = int((flyash_float)*0.9)+378    #393                   # last modify 22 december
            else:
                flyash_weight_int = int((flyash_float)*0.9)+378    # 392                  # previous constant value = 398  ;;; when add 398 the weight are 4 kg more than set point
        ...
        # ========= set cemen weight (flyash+cemen) =====
            ...
            current_amount = float(amount_string.get())
            if current_amount <= 0.5:
                cemen_weight_int = int((f1+c1)*0.9)+389              # 393         # last modify 22 december
            else:
                cemen_weight_int = int((f1+c1)*0.9)+389              # 395         # the previous constant is 392
        ...
        # ======== set water weight =============
            ...
            current_amount = float(amount_string.get())
            if current_amount <= 0.5:
                water_weight_int = int(water_weight_float*0.95)+368                        # y = 0.95x+341
            else:
                water_weight_int = int(water_weight_float*0.95)+368                         # previous constant = 357
        ...
        # ======= set chem1 weight =============
            ...
            chem1_weight_int = int(float(chem1_target_weight_string.get())*190) + int(chem_offset_float)        # y = 190x + 170 >> previous constant = 149
            if running:
        ...
        # ======= set chem2 weight (chem1+chem2) =============
            ...
            chem2_weight_int = int((c1+c2)*190) + int(chem_offset_float)              # 190x + 170                    previous constant = 155
            if running:
        ...
        # =========== check rock2 ======
         ...
         # ========== read rock2 weight ===========
            ...
            if in_loop:
                ...
                current_amount = float(amount_string.get())
                if current_amount <= 0.5:
                    state_delay = state_interval + 7500
                else:
                    state_delay = state_interval + 3500
                main_window.after(state_delay,main_controller)
        # =============== start mixed all aggregates here ========
            ...
            if running:
                ...
                #add_status(message)
            if in_loop:
                state_delay = state_interval + 12000
                main_window.after(state_delay,main_controller)
        # ============= pump chem to mixer ========================
        elif main_state == 315:
        ...
        # =========== refill flyash ================
            ...
            if running:
                ...
                if modbus_result.function_code < 0x80:
                    ...
                    if abs(flyash_weight-flyash_target) <= 3 or flyash_weight>=flyash_target:
                        main_state = 303
                    else:
                        main_state = 330
            else:
        ...
        #=========== refill cemen =================
            ...
            if running:
                modbus_result = client.read_holding_registers(address=1,count=1,unit=6)
                if modbus_result.function_code < 0x80:
                    present_cemen_weight = int(modbus_result.registers[0])
                    cemen_weight_target = float(cemen_target_weight_string.get())+ float(flyash_target_weight_string.get())
                    if abs(present_cemen_weight-cemen_weight_target) <= 3 or present_cemen_weight >= cemen_weight_target:
                        main_state = 309
                    else:
                        main_state = 351
            else:
        ...
        elif main_state == 351:                             # clear flags
            cemen_display_label.configure(bg='green')
            if running:
                modbus_result = client.write_coil(address=5,value=0,unit=2)
                if modbus_result.function_code < 0x80:
                    main_state = 352
            else:
                main_state = 352
            ...
            cemen_display_label.configure(bg='green')
            if running:
                modbus_result = client.write_coil(address=6,value=0,unit=2)
                if modbus_result.function_code < 0x80:
                    main_state = 353
            else:
                main_state = 353
            if in_loop:
                state_delay = state_interval + 500
                main_window.after(state_delay,main_controller)
        elif main_state == 353:          # ======= restart weighting  cemen =====
            cemen_display_label.configure(bg='green')
            if running:
                modbus_result = client.write_coil(address=5,value=1,unit=2)
                if modbus_result.function_code < 0x80:
...
main_font = font.Font(size=12,weight="bold")
